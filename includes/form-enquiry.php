...
(function($){
    $('#enquiry').submit(function(event){
        event.preventDefault(); // Cancell submit event when refleshing the site. Clicking on a link, prevent the link from following the URL

        var endpoint = '<?php echo admin_url( 'admin-ajax.php' ); ?>'; // load AJAX in WordPress

        var form = $('#enquiry').serialize(form);// get form inputs

        var formdata = new FormData;

        formdata.append('action', 'enquiry');
        formdata.append('nonce', '<?php echo wp_create_nonce( 'ajax-nonce' )?>'); // Make sure that form request comes from WP site
        ...
        $.ajax(endpoint,{
            ...
            success: function(res){
                ...
                $('#enquiry').trigger('reset'); // Clean up the form from filled inputs once the message is sendt...
                $('#enquiry').fadeIn(500);
            },
